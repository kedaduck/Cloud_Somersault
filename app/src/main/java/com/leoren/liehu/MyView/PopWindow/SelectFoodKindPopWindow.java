package com.leoren.liehu.MyView.PopWindow;
...
public abstract class SelectFoodKindPopWindow {

    private static final String TAG = "SelectFoodKindPopWindow";

    protected Context context;
    protected View contentView;
    protected PopupWindow instance;

    public SelectFoodKindPopWindow(Context c, int layoutRes, int w, int h) {
        Log.i(TAG, "SelectFoodKindPopWindow: " + c.toString().trim());
        context=c;
        ...
        instance = new PopupWindow(contentView, w, h, true);

        initWindow();
    }

    public View getContentView() { return contentView; }
    public PopupWindow getPopupWindow() { return instance; }
    ...
    public void showBashOfAnchor(View anchor, LayoutGravity layoutGravity, int xmerge, int ymerge) {
        ...
        instance.showAsDropDown(anchor, offset[0]+xmerge, offset[1]+ymerge);
    }

    public void showAsDropDown(View anchor, int xoff, int yoff) {
        instance.showAsDropDown(anchor, xoff, yoff);
    }

    public void showAtLocation(View parent, int gravity, int x, int y) {

        if(instance == null){
            Log.i(TAG, "showAtLocation: "+ "hahahahahhrenlieh");
        }
        if(parent == null){
            Log.i(TAG, "showAtLocation: renliehonh ");
        }
        instance.showAtLocation(parent, gravity, x, y);
    }
